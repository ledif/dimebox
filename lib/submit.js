...
module.exports = function(opt) {
  ...
  if (!opt.force && fs.existsSync(dirs.jobs(epoch, '.submitted.json'))) {
    log.error("This experiment has already been submitted.")
    log.error(`Invoke 'dimebox generate experiments/jobs/${epoch}/run.yml' to generate a new experiment from the same configuration.`)
    log.error("Alternatively, use --force to override.")
    process.exit(1)
  }
