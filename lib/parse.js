...
function parseResults(epoch, tag, parser) {
  ...
  const failedFiles = _.filter(parsedResults, result => { return result.length == 0 })
  ...
  const nonEmptyResults = _.filter(parsedResults, result => { return result.length > 0 })
  if (nonEmptyResults.length == 0)
    return

  // Turn each row into a map
  const rowsAsMaps = _.map(nonEmptyResults, row => {
    let m = new Map()
    row.map(kv => {
      if (m.has(kv[0])) {
        log.warn(chalk.red("WARNING:"), "file contains same key twice")
      } else {
        m.set(kv[0], kv[1])
      }
    })
  ...
  const keys = Array.from(rowsAsMaps[0].keys())

  // Get the values for all rows in the same order as the header
  const orderedRows = _.map(rowsAsMaps, map => {
    let row = []
    for (const key of keys) {
      if (map.has(key)) {
        row.push(map.get(key))
      } else {
        log.warn(chalk.red("WARNING:"), `file does not contain key '${key}'`)
        row.push("null")
      }
    }
    ...
    if (map.size > keys.size)
      log.warn(chalk.red("WARNING:"), "file has", map.size, "key-value pairs while header has ", keys.size, "keys")
