...
let argv = require('yargs')
  ...
  .command('submit <epoch>', 'Submit all jobs for an epoch', yargs => {
    ...
    .boolean('stagger')
    .describe('stagger', "Set up a chain of dependent jobs so that jobs will only be executed on at a time.")
    .boolean('v')
  })
...
if (argv._[0] == 'generate') {
  ...
  guard();
  submit(argv.machine, argv.epoch, argv['dry-run'], argv.stagger);
