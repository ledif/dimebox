...
function combinedResults(epoch) {
  ...
  const started = _.reject(stat.results, f => {
    return _.includes(stat.done, f) || _.includes(stat.failed, f)
  })

  // Waiting jobs are those that are not in any other category
  const waiting = _.reject(stat.expected, f => {
    return _.includes(stat.done, f) || _.includes(stat.failed, f) || _.includes(started, f)
  })


  let all = []

  stat.failed.map(r => { all.push({params: _.fromPairs(params(epoch, r)), state:"failed"}) })
  stat.done.map(r => { all.push({params: _.fromPairs(params(epoch, r)), state:"done"}) })
  started.map(r => { all.push({params: _.fromPairs(params(epoch, r)), state:"started"}) })
  waiting.map(r => { all.push({params: _.fromPairs(params(epoch, r)), state:"waiting"}) })
...
function currentStatus(epoch) {
  ...
  combined.results.map(x => {
    log.info(charForState(x.state), JSON.stringify(x.params))
  });
