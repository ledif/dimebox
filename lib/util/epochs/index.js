...
module.exports = {
   ...
   * TODO: combine this with status, lots of redundancy
   **/
  activities(epoch) {
    ...
    let started = fs.existsSync(startedDir) ? 
      filesInDirectory(startedDir) : []

    // Results that started but timed out
    const timedOut = _.filter(started, res => {
    ...
    contributeAll(all, failed, "failed")
    contributeAll(all, done,  "done")
    contributeAll(all, started, "started")
