...
function isFailedResult(str, failureStrings) {
  return /not found/.test(str) || /Error/.test(str) 
    || /No such file or directory/.test(str)
    || /exit codes/.test(str)
    || /exit signals: Killed/.test(str)
}
...
function experimentStatus(epoch) {
    ...
    try {
      ...
      failed = _.filter(results, (filename) => {
        const resultsContent = fs.readFileSync(`${resultsDir}/${filename}`)
        return isFailedResult(resultsContent, failureStrings)
      });
