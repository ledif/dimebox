...
function experimentStatus(epoch) {
    ...
    try {
      const results = _.reject(
        ...
        filename => { return filename.includes('.yml') || filename.startsWith(".") || filename === 'stderr' || filename === 'stdout'; }
      )

      numResults = results.length
      
      // Failed runs are in the .fail directory
      failed = fs.readdirSync(`${resultsDir}.fail`)
    } catch (e) {
      log.debug(e)
    }
