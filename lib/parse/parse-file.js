...
function aggregate(kvs, file, allowed) {
  ...
  _.mapKeys(grouped, (values, key) => {
    ...
    const st = _.mapValues(stats(floatValues), val => Number(val).toExponential(6))
    summarized.push([key+'_mean', st.mean])
    summarized.push([key+'_min', st.min])
    summarized.push([key+'_max', st.max])
    summarized.push([key+'_conf', st.conf])
    summarized.push([key+'_stddev', st.stddev])
  })
