...
module.exports = function (opt) {
  ...
  const sortColumns = opt.sortColumns.map(n => {return e => {return e.map.get(n)}})
  ...
  const rowsAsMaps = _.chain(nonEmptyResults).map(result => {
    let row = result.kvs
    let m = new Map()
    row.map(kv => {
      if (m.has(kv[0])) {
  ...
  }).orderBy(sortColumns, orders).value()
