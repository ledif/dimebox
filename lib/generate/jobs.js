...
function writeMetadata(options) {
  let md = {
    job: `${process.cwd()}/experiments/jobs/${options.epoch}/${options.jobName}.job`,
    p: options.p,
    depth: options.depth,
    params: {}
  }
  ...
  const mdFile = `${process.cwd()}/experiments/results/${options.epoch}/.md/${options.cmdID}.yml`;
...
function execCmd(epoch, cmdID, cmdString) {
  ...
  return `
    ...
    s=\${PIPESTATUS[0]}
    if [ $s -eq 0 ]; then
      touch experiments/results/${epoch}/.done/${cmdID}
    else
      touch experiments/results/${epoch}/.fail/${cmdID}
    fi
   `
}
