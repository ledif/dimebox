...
function dumpYAML(exp, epoch, printVC) {
  ...
  fs.writeFile(dirs.jobs(epoch, 'machine.yml'), yaml.safeDump(machineInfo), (err) => { if (err) throw err });
}

function createJobFiles(template, exp, expObjs, vc) {
  const epoch = moment().format('YYYYMMDD-HHmmss')

  createDirectories(epoch);

  const generated = jobs.generateJobs(template, exp, expObjs, epoch)
...
module.exports = function(machine, experiment, vc) {
  ...
  const expAndObjs = expfile.loadExpfile(experiment)
  const exp = expAndObjs.data
  const expObjs = expAndObjs.objects

  // Read machine config
  const machineConfig = config.machine(machine);

  createJobFiles(machineConfig.template, exp, expObjs, vc)
}
